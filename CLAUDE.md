...
- PostgreSQL owns catalog, authorization, lifecycle, governance, documentation, search, durable event, dead-letter, reconciliation, and audit state.
- Webhook events are reconciliation hints; workers always re-read current JFrog state.
...
- Keep blocking JFrog/JDBC adapters on bounded virtual threads; do not introduce WebFlux without a new architecture decision.
...
4. Verify catalog counts, database queue/DLQ, search, and worker health after the seeder's reconciliation completes.
