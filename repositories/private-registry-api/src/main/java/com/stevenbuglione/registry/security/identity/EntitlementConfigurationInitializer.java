...
public class EntitlementConfigurationInitializer implements ApplicationRunner {
    ...
    public void run(ApplicationArguments arguments) {
        ...
        if (!properties.apmGroupMappings().isBlank()) {
            for (var mapping : properties.apmGroupMappings().split(",")) {
                var parts = mapping.trim().split(":", 2);
                if (parts.length != 2 || !APM_ID.matcher(parts[0].trim()).matches()) {
                    throw new IllegalStateException("REGISTRY_ENTRA_APM_GROUPS must contain APM0000000:<group-object-id> pairs");
                }
    ...
    private void upsert(String groupObjectId, String apmId, String displayName, String role) {
        try {
