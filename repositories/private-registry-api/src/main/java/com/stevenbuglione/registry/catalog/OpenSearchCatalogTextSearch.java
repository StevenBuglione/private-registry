...
public class OpenSearchCatalogTextSearch implements CatalogTextSearch {
    ...
    @Override
    public List<String> findPackageIds(AccessContext accessContext, CatalogQuery query, int maximumResults) {
        try {
            var response = client.search(search -> search
                            .index(INDEX)
                            .size(maximumResults)
                            .source(source -> source.filter(filter -> filter.includes("id")))
                            .query(root -> root.bool(bool -> {
                                bool.must(clause -> clause.multiMatch(match -> match
                                        .query(query.q())
                                        .fields("name^5", "title^4", "namespace^3", "description", "keywords")));
                                if (query.kind() != null) {
                                    bool.filter(clause -> clause.term(term -> term
                                            .field("kind")
                                            .value(FieldValue.of(query.kind().jsonValue()))));
                                }
                                if (query.provider() != null) {
                                    bool.filter(clause -> clause.bool(provider -> provider
                                            .should(name -> name.term(term -> term
                                                    .field("name.keyword")
                                                    .value(FieldValue.of(query.provider()))))
                                            .should(target -> target.term(term -> term
                                                    .field("target")
                                                    .value(FieldValue.of(query.provider()))))
                                ...
                                if (query.lifecycle() != null) {
                                    bool.filter(clause -> clause.term(term -> term
                                            .field("lifecycle")
                                            .value(FieldValue.of(query.lifecycle()))));
                                }
                                if (query.risk() != null) {
                                    bool.filter(clause -> clause.term(term -> term
                                            .field("risk_tier")
                                ...
                                var allowedApmIds = query.apmId() == null
                                        ? accessContext.apmIds()
                                        : java.util.Set.of(query.apmId());
                                if (!accessContext.registryAdmin() || query.apmId() != null) {
                                    bool.filter(clause -> clause.terms(terms -> terms
                                            .field("apm_ids")
                                            .terms(values -> values.value(allowedApmIds.stream()
                                                    .map(FieldValue::of)
                                                    .toList()))));
                                }
                                return bool;
