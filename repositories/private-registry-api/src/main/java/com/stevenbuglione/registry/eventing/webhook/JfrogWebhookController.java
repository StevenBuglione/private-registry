...
public class JfrogWebhookController {
    ...
    public ResponseEntity<Map<String, String>> receive(
            ...
            @RequestHeader(value = "X-JFrog-Event-Id", required = false) String eventId,
            @RequestHeader(value = "X-Correlation-Id", required = false) String correlationId) {
        validateEnvelope(payload, signature, origin, subscriptionId);
        var headers = new JfrogWebhookParser.WebhookHeaders(
                eventId,
                origin,
                subscriptionId,
    ...
    private void validateEnvelope(byte[] payload, String signature, String origin, String subscriptionId) {
        ...
        if (properties.secret().isBlank()) {
            throw new ResponseStatusException(HttpStatus.SERVICE_UNAVAILABLE, "Webhook signing secret is not configured");
        }
        if (properties.allowedOrigins().isEmpty() || properties.subscriptionId().isBlank()) {
            throw new ResponseStatusException(
                    HttpStatus.SERVICE_UNAVAILABLE, "Webhook origin and subscription allowlists are not configured");
        }
        ...
        if (!properties.subscriptionId().equals(subscriptionId)) {
            throw new ResponseStatusException(HttpStatus.UNAUTHORIZED, "Webhook subscription is not allowed");
        }
