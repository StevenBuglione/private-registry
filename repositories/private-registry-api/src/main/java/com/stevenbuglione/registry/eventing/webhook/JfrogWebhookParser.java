...
public class JfrogWebhookParser {
  ...
  CatalogArtifactChanged parse(byte[] payload, WebhookHeaders headers) {
    ...
    var repository =
        requiredFirstText(
            "repository", data.path("repo_key"), data.path("repository"), root.path("repo_key"));
    var path =
        requiredFirstText("path", data.path("path"), data.path("item_path"), root.path("path"));
    var eventType =
        requiredFirstText(
            "event type", root.path("event_type"), root.path("type"), data.path("event_type"));
    var occurredAt =
        firstText(null, root.path("occurred_at"), root.path("timestamp"), data.path("timestamp"));
    var correlationId = firstText(headers.correlationId(), root.path("correlation_id"));
    var properties = new LinkedHashMap<String, String>();
    data.path("properties")
        .forEachEntry(
            (key, value) ->
                properties.put(key, value.isValueNode() ? value.asString() : value.toString()));
    return new CatalogArtifactChanged(
        ...
        path,
        instant(occurredAt),
        correlationId == null ? resolvedEventId : correlationId,
        properties);
  }
  ...
  private static @Nullable String firstText(@Nullable String fallback, JsonNode... candidates) {
  ...
  private static String requiredFirstText(String field, JsonNode... candidates) {
    var value = firstText(null, candidates);
    if (value == null) {
      throw new IllegalArgumentException("JFrog webhook " + field + " is required");
    }
    return value;
  }
