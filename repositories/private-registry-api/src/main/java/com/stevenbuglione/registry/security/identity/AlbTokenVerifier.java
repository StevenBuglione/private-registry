...
public class AlbTokenVerifier {
  ...
  public VerifiedIdentity verify(String encodedToken, String identityHeader) {
    ...
    if (encodedToken == null
        || encodedToken.isBlank()
        || encodedToken.length() > MAX_TOKEN_LENGTH) {
    ...
    try {
      ...
      if (!subject.equals(identityHeader)) {
        throw new BadCredentialsException(
            "The ALB identity headers do not identify the same subject");
      }
      return new VerifiedIdentity(
          subject,
          textOrDefault(claims, "name", subject),
          nullableText(claims, "email", nullableText(claims, "preferred_username", null)));
  ...
  private static String textOrDefault(JsonNode claims, String name, String defaultValue) {
    var value = claims.path(name);
    return value.isString() && !value.stringValue().isBlank() ? value.stringValue() : defaultValue;
  }

  private static @Nullable String nullableText(
      JsonNode claims, String name, @Nullable String defaultValue) {
    var value = claims.path(name);
    return value.isString() && !value.stringValue().isBlank() ? value.stringValue() : defaultValue;
  }
  ...
  private ECPublicKey publicKey(String keyId) {
    try {
      ...
      throw new BadCredentialsException(
          "The ALB identity token signing key is unavailable",
          java.util.Objects.requireNonNull(exception.getCause()));
    }
  }
  ...
  public record VerifiedIdentity(String subject, String displayName, @Nullable String email) {}
