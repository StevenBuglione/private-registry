...
public class GraphMembershipClient {
  ...
  public GraphMembershipClient(
      HttpClient httpClient, ObjectMapper objectMapper, IdentityProperties properties) {
  ...
  GraphMembershipClient(
    ...
    this.cache =
        Caffeine.newBuilder().expireAfterWrite(properties.membershipCacheTtl()).buildAsync();
  }

  public Set<String> checkMemberships(
      String subject, String delegatedAccessToken, Set<String> candidateGroupIds) {
    ...
    if (delegatedAccessToken == null || delegatedAccessToken.isBlank()) {
      throw new IdentityProviderUnavailableException(
          "A delegated Microsoft Graph access token is required");
    }
    ...
    try {
      ...
      throw new IdentityProviderUnavailableException(
          "Microsoft Graph membership lookup failed", cause == null ? exception : cause);
    }
  }
  ...
  private Set<String> fetchMemberships(String accessToken, List<String> candidates) {
    ...
    for (var offset = 0; offset < candidates.size(); offset += MAX_GROUPS_PER_REQUEST) {
      var batch =
          candidates.subList(offset, Math.min(offset + MAX_GROUPS_PER_REQUEST, candidates.size()));
      var request =
          HttpRequest.newBuilder(properties.graphEndpoint())
      ...
      try {
        ...
        if (!values.isArray()) {
          throw new IdentityProviderUnavailableException(
              "Microsoft Graph returned an invalid membership response");
        }
        values.forEach(
            value -> {
        ...
        throw new IdentityProviderUnavailableException(
            "Microsoft Graph membership lookup was interrupted", exception);
      } catch (IOException exception) {
        throw new IdentityProviderUnavailableException(
            "Microsoft Graph membership lookup failed", exception);
      }
    }
  ...
  private String requestBody(List<String> batch) {
    try {
      ...
      throw new IdentityProviderUnavailableException(
          "Unable to create Microsoft Graph membership request", exception);
    }
  }
